...
/**
 * A generic tree item that represents a team object in the team outline and allows the author
 * to interact with it
 * 
 * @author nroberts
 * @param <T> the type of team object that this item represents
 */
public abstract class TeamObjectTreeItem<T extends Serializable> extends TreeItem {
    /** The logger for the class */
    private static Logger logger = Logger.getLogger(TeamObjectTreeItem.class.getName());
    ...
    /** The radio button group name to use when only one team member can be picked */
    private static final String TEAM_MEMBER_SELECTOR_GROUP = "TeamMemberSelectorGroup";

    
    /** The different modes that can be used to pick teams and/or team members within tree items and their children */
    public static enum PickMode{
        
        /** A mode specifying that no teams or team members should be pickable */
        NONE,
        
        /** A mode specifying that the author should only be able to pick a single team member */
        SINGLE,
        
        /** A mode specifying that the author should be able to pick multiple teams and team members */
        MULTIPLE
    }
    
    /** The tree item currently being dragged, if the user is currently dragging an item */
    private static TeamObjectTreeItem<? extends Serializable> itemBeingDragged = null;
    ...
    /** The team object that this tree item represents */
    protected T object;
    ...
    /** 
     * The check box used to pick this item's team or team member. 
     * <br/><br/>
     * If this item is set to is {@link PickMode#SINGLE} mode and represents a {@link TeamMember}, 
    ...
    protected FlowPanel buttonPanel = new FlowPanel();
    ...
    /** The mode that this item is currently using to allow the author to pick team members*/
    ...
    /** 
     * Whether teams represented by this item and its children should be able to be selected without selecting
     * their children, allowing users to pick teams without their children.
     */
    private boolean teamPickEnabled = false;
     ...
     */
    protected TeamObjectTreeItem(T object){
        this(object, false);
    }
     ...
     */
    protected TeamObjectTreeItem(T object, final boolean draggable){
        ...
        if (draggable) {
            // add an icon to indicate that team tree items can be dragged
        ...
        nameLabel.addValueChangeHandler(new ValueChangeHandler<String>() {
            ...
            public void onValueChange(ValueChangeEvent<String> event) {
                ...
                Serializable teamObject = getTeamObject();
                
                String name = ScenarioElementUtil.getObjectName(teamObject);
                ...
                } else if (!isObjectNameValid(teamObject, newName)) {

                    Notify.notify("Another Team or Team member is already named '" + newName +"'.", NotifyType.DANGER);
                    nameLabel.setValue(name);

                    return;
                }
                
                String oldName = ScenarioElementUtil.getObjectName(teamObject);
                ScenarioElementUtil.setObjectName(teamObject, newName);
                
                nameLabel.setValue(newName);
                onRename();
                
                // update references to this team unit whenever its name changes
        ...
        if(draggable) {
            ...
            //use D3's drag behavior to allow team tree items to be dragged (similar to nodes in the course tree)
            ...
            elementSelection.call(D3.behavior().drag()
                ...
                .on(DragEventType.DRAG, new DatumFunction<Void>() {
                    ...
                    public Void apply(final Element context, Value d, int index) {
                        ...
                        if(itemBeingDragged == null && isDraggingEnabled) {
                            ...
                            itemBeingDragged = TeamObjectTreeItem.this;
                        }
                ...
                public Void apply(Element context, Value d, int index) {
                     ...
                     * 
                     * Note that adding more <img> elements to team object tree items in the future may break this logic.
                     */
                ...
                public Void apply(Element context, Value d, int index) {
                    ...
                    if(!TeamObjectTreeItem.this.equals(getTree().getSelectedItem())) {
                        getTree().setSelectedItem(TeamObjectTreeItem.this, true);
                    }
        ...
        elementSelection.on("mouseover", new DatumFunction<Void>() {
            ...
            public Void apply(Element context, Value d, int index) {
                ...
                if(itemBeingDragged != null) {
                    ...
                    //need to ensure that the team outline doesn't handle dropping this item
                    itemBeingDragged = null;
                }
    ...
    /**
     * Gets the team object that this tree item represents. Can't be null.
     * 
     * @return the team object that this tree item represents
     */
    public T getTeamObject() {
        return object;
    }
    ...
    @Override
    public TeamObjectTreeItem<?> getParentItem() {
        TreeItem parentItem = super.getParentItem();
        if (parentItem == null) {
            return null;
        } else if (!(parentItem instanceof TeamObjectTreeItem<?>)) {
            logger.severe("Trying to get the parent item that is not of type '"
                    + TeamObjectTreeItem.class.getSimpleName() + "'");
            return null;
        }

        return (TeamObjectTreeItem<?>) parentItem;
    }
    
    /**
     * Handles when another team tree item is dropped on top of this one. This method will not do anything by default, but
     * classes extending this method may use it to handle when specific tree items are dragged.
     * 
     * @param dragged the team item that was dragged and dropped on this item
     */
    protected void onDrop(TeamObjectTreeItem<? extends Serializable> dragged) {
        //by default, don't do anything when team objects are dropped on top of each other
    }
     ...
     */
    public static TeamObjectTreeItem<? extends Serializable> getItemBeingDragged() {
     ...
     * 
     * @param pickMode the mode that can be used to pick teams and/or team members within tree items and their children
     */
    public void setPickMode(TeamObjectTreeItem.PickMode pickMode) {
        
        if(!PickMode.NONE.equals(pickMode)) {
            
            boolean isNewCheckBox = false;
            
            if(PickMode.SINGLE.equals(pickMode) && object instanceof TeamMember && !(checkBox instanceof RadioButton)) {
                
                //provide a radio button that the author can use to pick only one team member item
                checkBox = new RadioButton(TEAM_MEMBER_SELECTOR_GROUP);
        ...
        for(int i = 0; i < getChildCount(); i++) {
            ((TeamObjectTreeItem<?>) getChild(i)).setPickMode(pickMode);
        }
    }
    
    /**
     * Toggles whether or not this item is currently picked, as if the user clicked on this
     * team object's check box.
     */
    public void togglePicked() {
        
        boolean oldValue = checkBox.getValue() != null ? checkBox.getValue() : false;
        boolean newValue = false;
        boolean pickTeamOnly = false;
        
        if((teamPickEnabled && !(object instanceof TeamMember))) {
            
            if(!oldValue) {
                
                //if this team is not picked, pick it but not its children
                pickTeamOnly = true;
                newValue = !oldValue;
                ...
                if(hasAnyPickedImmediateChildren) {
                    ...
                    //if any of this team's immediate children are picked, unpick this team
                    newValue = !oldValue;
                    
                } else {
                    
                    //if this team is picked without its children, pick its children
                    newValue = true;
                }
            }
        ...
        if(isPickable()) {
            ...
            if(teamPickEnabled && pickTeamOnly) {
                
                //unpick this team's children if only the team itself should be picked
                setChildrenPicked(false);
                
            } else {
                
                //otherwise, pick/unpick this team's children normally
                setChildrenPicked(newValue);
            }
            
            //if the author unchecks this item, need to make sure its parents are unchecked too
            TeamObjectTreeItem<?> parent = TeamObjectTreeItem.this.getParentItem();
    ...
    private void setChildrenPicked(boolean picked) {
        ...
        for(int i = 0; i < getChildCount(); i++) {
            ...
            TeamObjectTreeItem<?> item = (TeamObjectTreeItem<?>) getChild(i);
            
            if(item.isPickable()) {
                item.checkBox.setValue(picked);
            }
            
            if(!picked || !teamPickEnabled) {
                
                //only propagate this item's pick state to its children if it is not picked or team picking is disabled
    ...
    /**
     * Gets the names of the top-most teams and team members that have been picked. If a parent team is picked, its children's
     * names will be excluded from the list.
     * 
     * @return the names of the top-most teams and team members that have been picked
     */
    public List<String> getPickedTeamObjectNames() {
        ...
        for(int i = 0; i < getChildCount(); i++) {
            checkedNames.addAll(((TeamObjectTreeItem<?>) getChild(i)).getPickedTeamObjectNames());
        }
    ...
    /**
     * Sets the names of the top-most teams and team members that should be checked. If one of the provided names belongs to
     * a parent team with children, all of its children will be selected automatically.
     * 
     * @param teamNames the names of the teams and team members that should be checked
     */
    public void setPickedTeamObjectNames(Collection<String> teamNames) {

        boolean hasNameToCheck = teamNames != null
                && teamNames.contains(ScenarioElementUtil.getObjectName(getTeamObject()));
        ...
        if (hasNameToCheck) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                TeamObjectTreeItem<?> item = (TeamObjectTreeItem<?>) getChild(i);
                item.setPickedTeamObjectNames(teamNames);
            }
    ...
    /**
     * Gets the names of all the team members that have been picked.
     * 
     * @return the names of the team members that have been picked.
     */
    public List<String> getAllPickedTeamMemberNames() {

        List<String> checkedNames = new ArrayList<>();

        if (Boolean.TRUE.equals(isPicked()) && object instanceof TeamMember) {
            checkedNames.add(nameLabel.getValue());
        }

        for (int i = 0; i < getChildCount(); i++) {
            final TeamObjectTreeItem<?> treeItem = (TeamObjectTreeItem<?>) getChild(i);
            checkedNames.addAll(treeItem.getAllPickedTeamMemberNames());
        }
    ...
    public List<String> getAllTeamMemberNames() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            final TeamObjectTreeItem<?> treeItem = (TeamObjectTreeItem<?>) getChild(i);
            names.addAll(treeItem.getAllTeamMemberNames());
        }
    ...
    /**
     * Executes logic whenever the user uses this item to rename its represented team object. By default, this method will do
    ...
    public String toString() {
        return new StringBuilder()
                .append("[TeamObjectTreeItem: ")
                .append("object = ").append(getTeamObject())
                .append("]").toString();
    }
    
    /**
     * If this team object contains the specified text, modifies its styling to indicate such. If this team object has any children 
     ...
     * @return whether or not this team object or any of its children contain the selected text
     */
    ...
    /**
     * If this team object contains the specified search terms, modifies its styling to indicate such. If this team object has any children 
    ...
    private boolean sortBySearchTerms(String[] searchTerms) {
        ...
        if(searchTerms != null) {
            ...
            String teamObjectName = getNameLabel().getValue();
            
            //check if each team object contains the necessary search terms
            for(int i = 0; i < searchTerms.length; i++) {
                
                if(teamObjectName == null || !teamObjectName.toLowerCase().contains(searchTerms[i])) {
                    missingTerm = true;
                    break;
                }
            }
    ...
    /**
     * Stops dragging the team object currently being dragged, if there is such an object. If no team object is 
    ...
    public void cleanUpTooltips() {
        ...
        for(int i = 0; i < getChildCount(); i++) {
            ((TeamObjectTreeItem<?>) getChild(i)).cleanUpTooltips();
        }
    }

    /**
     * Retrieve the check box used to pick this item's team or team member.
     * 
     ...
     */
    protected abstract boolean isObjectNameValid(Serializable object, String name);
    
    /**
     * Updates all references to the team or team member with the old name to
     ...
     */
    protected abstract void updateTeamReferences(String oldName, String newName);
    
    
    /**
     * Gets the names of all the team members that have been picked, directly or indirectly via their parents, 
     * and returns a mapping that an be used to quickly reference the selected parent roles nearest to them 
     * in the tree.
     * 
     * @return a mapping from the names of the team members that have been picked to their nearest selected
     * parent nodes. Will not be null.
     */
    public Map<String, String> getMembersToNearestPickedRoles() {
        return getMembersToNearestPickedRoles(null, null);
    }
     ...
     * in the tree.
     * 
     * @param map the map to populate with team member names and their nearest selected parent roles in the tree. 
     * If null, a new map will be created.
     * @return a mapping from the names of the team members that have been picked to their nearest selected
     * parent nodes. Will not be null.
     */
    public Map<String, String> getMembersToNearestPickedRoles(Map<String, String> memberToNearestSelectedRole, String nearestSelectedParent) {

        if(memberToNearestSelectedRole == null) {
            memberToNearestSelectedRole = new HashMap<>();
        }
        ...
        if(isPickable()) {
            if(object instanceof TeamMember) {
                ...
                if(name != null) {
                    ...
                    if(Boolean.TRUE.equals(isPicked())) {
                        memberToNearestSelectedRole.put(name, name);
                        
                    } else if(nearestSelectedParent != null){
                        memberToNearestSelectedRole.put(name, nearestSelectedParent);
                    }
                }
                ...
                for (int i = 0; i < getChildCount(); i++) {
                    ((TeamObjectTreeItem<?>) getChild(i)).getMembersToNearestPickedRoles(
                            memberToNearestSelectedRole, 
                            Boolean.TRUE.equals(isPicked()) ? getName() : nearestSelectedParent
                    );
                }
            }
        }
    ...
    /**
     * Sets whether the user should be able to pick teams in this team hierarchy without automatically
     * picking their children.
     * 
     * @param enabled whether team picking should be enabled. Defaults to false;
     */
    public void setTeamPickEnabled(boolean enabled) {
        this.teamPickEnabled = enabled;
        
        for(int i = 0; i < getChildCount(); i++) {
            ((TeamObjectTreeItem<?>) getChild(i)).setTeamPickEnabled(enabled);
        }
    }
    
    /**
     * Gets the name of this team object's role in the team hierarchy
     * 
     * @return the team object's role name. Cannot be null.
     */
     ...
     * a team is selected without its children when team picking is enabled.
     */
    private void refreshCheckBoxStyling() {
        
        if(isPickable() && !(object instanceof TeamMember) && teamPickEnabled) {
            ...
            if(Boolean.TRUE.equals(isPicked()) && !hasPickedImmediateChild) {
                ...
                //a team item is selected without any selected children, so apply styling to distinguish it
