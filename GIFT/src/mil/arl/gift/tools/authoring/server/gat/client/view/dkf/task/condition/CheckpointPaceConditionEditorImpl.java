...
/**
 * The Class CheckpointPaceConditionEditor.
 */
public class CheckpointPaceConditionEditorImpl extends ConditionInputPanel<CheckpointPaceCondition> {

    /** The logger for the class */
    private static final Logger logger = Logger.getLogger(CheckpointPaceConditionEditorImpl.class.getName());

    /** The ui binder. */
    private static CheckpointPaceConditionEditorUiBinder uiBinder = GWT
            .create(CheckpointPaceConditionEditorUiBinder.class);

    /**
     * The Interface CheckpointPaceConditionEditorUiBinder.
     */
    interface CheckpointPaceConditionEditorUiBinder extends UiBinder<Widget, CheckpointPaceConditionEditorImpl> {
    }

    /** The data grid. */
    @UiField(provided = true)
    public ItemListEditor<Checkpoint> checkpointsEditor = new ItemListEditor<>(new CheckpointEditor(this));
     ...
     */
    public CheckpointPaceConditionEditorImpl() {
        if (logger.isLoggable(Level.FINE)) {
            logger.fine("CheckpointPaceConditionEditorImpl()");
        }

        initWidget(uiBinder.createAndBindUi(this));

        Widget checkpointCreateButton = checkpointsEditor.addCreateListAction("Click here to add a new checkpoint",
                new CreateListAction<Checkpoint>() {
        ...
        checkpointsEditor.setDraggable(true);
        ...
        checkpointValidation = new WidgetValidationStatus(checkpointCreateButton,
                "You must have at least 1 checkpoint to measure pace. Please add a new checkpoint.");
