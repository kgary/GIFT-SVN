...
public class FlexTableUtil {
    /**
     * Copy an entire FlexTable row from one FlexTable to another. Each element is copied by creating a
    ...
    /**
     * Move an entire FlexTable row from one FlexTable to another. Elements are moved by attempting to
     * call {@link FlexTable#getWidget(int, int)} on the source table. If no widget is found
     * (because <code>null</code> is returned), a new {@link HTML} is created instead by calling
