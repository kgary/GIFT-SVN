 ...
 * @author nroberts
 */
public class ExperimentCourseExportSummaryWidget extends Composite {
	...
	interface BsExportSummaryWidgetUiBinder extends
			UiBinder<Widget, ExperimentCourseExportSummaryWidget> {
	}
	...
	public ExperimentCourseExportSummaryWidget() {
		initWidget(uiBinder.createAndBindUi(this));
	}

	/**
	 * Sets the experiment from which the course should be exported. This is used to calculate the export size.
	 * 
	 * @param experiment
	 */
	public void setExperiment(DataCollectionItem experiment){
		...
		if(experiment != null && experiment.getId() != null && experiment.getCourseFolder() != null){
			...
			sb.appendEscaped(experiment.getCourseFolder());
			sb.appendHtmlConstant("</li>");
			...
			dashboardService.calculateExperimentCourseExportSize(UiManager.getInstance().getUserName(), experiment.getId(), new AsyncCallback<DoubleResponse>() {
