 ...
 * this work.
 */
package mil.arl.gift.gateway.interop.sudoku;
 ...
 *
 */
public class SudokuTAPluginInterface extends AbstractInteropInterface {
    
    /** Instance of the logger */
    private static Logger logger = LoggerFactory.getLogger(SudokuTAPluginInterface.class);
    ...
    static{
        ...
        REQ_TRAINING_APPS.add(TrainingApplicationEnum.SUDOKU); 
    }
    ...
    private static final String WINDOW_TITLE = "Sudoku Conquest";
    ...
    private static final File APP_EXE = 
            new File(".." + File.separator + "Training.Apps" + File.separator + "Sudoku" + File.separator + "Application" + File.separator +
                    "Sudoku_final_version" + File.separator + "bin" + File.separator + "Debug" + File.separator + "SudokuExperiment.exe");
...
//    private static final String TA_LOAD_METHOD_NAME = "load";
    ...
    private static SudokuTAPluginInterface instance = null;
     ...
     *
     */
    public static class SudokuTAPluginXMLRPC{
        
        public Integer start(){
            
            StartResume startResume = new StartResume(0, 0, 0);
            sendMessageToGIFT(startResume, MessageTypeEnum.START_RESUME);   
        ...
        public Integer showSurvey(){
            
            SimpleExampleState state = new SimpleExampleState("showSurvey");
            sendMessageToGIFT(state, MessageTypeEnum.SIMPLE_EXAMPLE_STATE);
...
//        }
     ...
     */
    public SudokuTAPluginInterface(String name){
        super(name, true); 
    ...
    public boolean configure(Serializable config) throws ConfigurationException {
        ...
        if (config instanceof generated.gateway.XMLRPC) {
            ...
            try{
                server = ApplicationConnectionFactory.createXMLRPCServer(pluginConfig.getServerNetworkPort(), SudokuTAPluginXMLRPC.class);
            }catch(Exception e){
                throw new ConfigurationException("Failed to create the XML RPC server.", e.getMessage(), e);
            }
//            client = new XMLRPCClient(pluginConfig.getExternalServerNetworkAddress(), pluginConfig.getExternalServerNetworkPort());
    ...
    public void setEnabled(boolean value) throws ConfigurationException{
        ...
        if(!isEnabledAlready && value){
            ...
            try {
			//MH: uncomment this line to have GIFT launch Sudoku VB application
...
//                Object returnObj = callRPCMethod(TA_LOAD_METHOD_NAME, args.getScenarioName(), errorMsg);       
...
//                callRPCMethod(TA_BLOB_METHOD_NAME, "Pause message received", errorMsg);
                ...
                try{
                    ...
                    GatewayModuleUtils.minimizeWindow(AHK_CLASS, WINDOW_TITLE);
                }catch(ConfigurationException e){
                    logger.error("Failed to set the Sudoku window as NOT the foreground window.", e);
                }
                
            }else if(siman.getSimanTypeEnum() == SimanTypeEnum.RESTART){
                
//                callRPCMethod(TA_BLOB_METHOD_NAME, "Restart message received", errorMsg);
                
            }else if(siman.getSimanTypeEnum() == SimanTypeEnum.RESUME){
                
//                callRPCMethod(TA_BLOB_METHOD_NAME, "Resume message received", errorMsg);
                ...
                try{
                    ...
                    logger.error("Failed to set the Sudoku window as the foreground window.", e);
                }
                
            }else if(siman.getSimanTypeEnum() == SimanTypeEnum.START){
                
//                callRPCMethod(TA_BLOB_METHOD_NAME, "Start message received", errorMsg);
				...
				//MH: Sudoku TA will be closing itself, GIFT doesn't need to send the stop in this situation
                
                //callRPCMethod(TA_CLOSE_APP_METHOD_NAME, new Vector<>(0), null);
                
                //setting the "Example Training Application" program's window to be in the background
                //This is optional but adds an automated look and feel
                //GatewayModuleUtils.setAlwaysOnTop(AHK_CLASS, WINDOW_TITLE, false);
                //GatewayModuleUtils.minimizeWindow(AHK_CLASS, WINDOW_TITLE);
