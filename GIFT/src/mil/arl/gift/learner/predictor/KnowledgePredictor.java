...
/**
 * This class predicts the next learner state for Knowledge.
 * 
 * @author mhoffman
 *
 */
public class KnowledgePredictor extends AbstractPredictor {

	/** instance of the logger */
    private static Logger logger = LoggerFactory.getLogger(KnowledgePredictor.class);
	 ...
	 */
	public KnowledgePredictor(AbstractClassifier classifier){
		super(classifier);
	}
	...
	public boolean updateState(){
		...
		AbstractAssessment assessment = (AbstractAssessment)((KnowledgeClassifier)classifier).getCurrentData();
		...
		if(assessment instanceof TaskAssessment){
	        ...
	        ExpertiseLevelEnum nextLevel = null;
	        LearnerStateAttribute state = (LearnerStateAttribute)classifier.getState();
	        ExpertiseLevelEnum currLevel = (ExpertiseLevelEnum)state.getPredicted();
	        ...
	        if(assessment.getAssessmentLevel() != AssessmentLevelEnum.UNKNOWN){
	            ...
	            for(ConceptAssessment ass : conceptAssessments){
	                ...
	                if(score >= 3){
	                    ...
	                    nextLevel = ExpertiseLevelEnum.EXPERT;
	                    break;
	                }else if(score <= -3){
	                    //reached the low level
	                    nextLevel = ExpertiseLevelEnum.NOVICE;
	                    break;
	                }else{
