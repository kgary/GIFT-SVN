 ...
 *
 */
public class TaskAppender extends AbstractStatementAppender {
     ...
     */
    public TaskAppender(TaskPerformanceState state) throws LmsXapiProfileException {
    ...
    public AbstractGiftStatement appendToStatement(AbstractGiftStatement statement) throws LmsXapiAppenderException {
        try {
            statement.addOtherActivity(taskATC.asActivity(task));
        } catch (LmsXapiActivityException e) {
            throw new LmsXapiAppenderException("unable to append task other activity!", e);
        }
        return statement;
    }
}
